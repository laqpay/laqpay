...
- Add new wallet type `bip44` for hierarchical deterministic (HD) wallets obeying the bip44 protocol.
  The Laqpay bip44 `coin` number is `8000`.
  ...
  More details are explained in https://github.com/laqpay/laqpay/wiki/Wallet-File-Formats-and-Types
...
- #2373 Fix and clean-up further panics with various `laqpay-cli` commands (lastBlocks, checkdb) which were not correctly handling arguments.
...
- When sending coins in the UI, the user can choose to send in LAQ, or the equivalent amount of LAQ in USD
...
- Add CLI `checkDBDecoding` command to verify the `laqencoder`-generated binary decoders match the reflect-based decoder
...
- #2348 Fix panic in `laqpay-cli` `transaction` command if no (zero) arguments are passed. Exactly one argument is expected.
...
- Use [`laqencoder`](https://github.com/laqpay/laqencoder)-generated binary encoders/decoders for network and database data, instead of the reflect-based encoders/decoders in `cipher/encoder`.
...
- Move package `github.com/laqpay/laqpay/src/cipher/go-bip39` to `github.com/laqpay/laqpay/src/cipher/bip39`
...
- `/api/v1/explorer/address` endpoint (use `GET /api/v1/transactions?verbose=1` instead). See https://github.com/laqpay/laqpay/blob/develop/src/api/README.md#migrating-from--api-v1-explorer-address
- The unversioned REST API (the `-enable-unversioned-api` is removed, prefix your API requests with `/api/v1` if they don't have an `/api/vx` prefix already). See https://github.com/laqpay/laqpay/blob/develop/src/api/README.md#migrating-from-the-unversioned-api
- JSON-RPC 2.0 interface (this is no longer used by the CLI tool, and the REST API supports everything the JSON-RPC 2.0 API does). See https://github.com/laqpay/laqpay/blob/develop/src/api/README.md#migrating-from-the-jsonrpc-api
- `/api/v1/wallet/spend` endpoint (use `POST /api/v1/wallet/transaction` followed by `POST /api/v1/injectTransaction` instead). See https://github.com/laqpay/laqpay/blob/develop/src/api/README.md#migrating-from--api-v1-spend
...
- `/api/v1/health` will return correct build info when running Docker containers based on `laqpay/laqpay` mainnet image.
- #2083, Windows desktop wallet sometimes shows "Error#1" on start

### Changed

- Extend URI specification to allow plain addresses (i.e. without a `laqpay:` prefix)
- Switch `laqpay-cli` from `urfave/cli` to `spf13/cobra`.
...
- Remove liblaqpay source code. Migrated to https://github.com/laqpay/liblaqpay
...
- JSON-RPC 2.0 interface (this is no longer used by the CLI tool, and the REST API supports everything the JSON-RPC 2.0 API does). See https://github.com/laqpay/laqpay/blob/develop/src/api/README.md#migrating-from-the-jsonrpc-api
- `/api/v1/wallet/spend` endpoint (use `POST /api/v1/wallet/transaction` followed by `POST /api/v1/injectTransaction` instead). See https://github.com/laqpay/laqpay/blob/develop/src/api/README.md#migrating-from--api-v1-spend
- The unversioned REST API (the `-enable-unversioned-api` option will be removed, prefix your API requests with `/api/v1`). See https://github.com/laqpay/laqpay/blob/develop/src/api/README.md#migrating-from-the-unversioned-api
- `/api/v1/explorer/address` endpoint (use `GET /api/v1/transactions?verbose=1` instead). See https://github.com/laqpay/laqpay/blob/develop/src/api/README.md#migrating-from--api-v1-explorer-address
...
- Database verification will only be performed once when upgrading to the next version. Verification will not be performed on subsequent upgrades unless necessary. To force verification, use `-verify-db=true`. Note that it is unsafe to downgrade the laqpay node without erasing the database first.
...
- Daemon configured builds will be available on the [releases](https://github.com/laqpay/laqpay/releases) page. The builds available for previous versions are configured for desktop client use.
- `laqpay-cli` builds will be available on the [releases](https://github.com/laqpay/laqpay/releases) page.
...
- Complete support for `cipher` package in `liblaqpay` C API.
- Add `coin`, `wallet`, `util/droplet` and `util/fee` methods as part of `liblaqpay` C API
- Add `make update-golden-files` to `Makefile`
- Add CLI `richlist` command
- Add `util/droplet` and `util/fee` API's as part of `liblaqpay`
...
- Fixed autogenerated HTTPS certs. Certs are now self-signed ECDSA certs, valid for 10 years, valid for localhost and all public interfaces found on the machine. The default cert and key are renamed from cert.pem, key.pem to laqpayd.cert, laqpayd.key
...
- `go run cmd/laqpay/laqpay.go` will have exit status 1 on failure and exit status 2 on panic
...
- liblaqpay 0.0.1 released with bindings for cipher/address, cipher/hash, cipher/crypto, cli/create_rawtx
...
- Add Dockerfile in `docker/images/dev-dind` to build a docker in docker image based on laqpaydev-cli.
...
- Write [specification for laqpay URIs](https://github.com/laqpay/laqpay#uri-specification) (based upon bip21)
...
- For electron clients, download a list of peers from https://downloads.laqpay.net/blockchain/peers.txt by default
...
- Add `/logs` api to filter laqpay logs, so that we can add a debug panel to the GUI wallet to show logs
...
- CLI's `walletHistory` command prints amounts as fixed-point decimal strings. Previously, it printed amounts as integers representing whole laqpay amounts, and did not support droplets / fractional laqpays.
...
- #383 Error during installation from laqpay source code
...
[0.26.0]: https://github.com/laqpay/laqpay/compare/v0.25.1...v0.26.0
[0.25.1]: https://github.com/laqpay/laqpay/compare/v0.25.0...v0.25.1
[0.25.0]: https://github.com/laqpay/laqpay/compare/v0.24.1...v0.25.0
[0.24.1]: https://github.com/laqpay/laqpay/compare/v0.24.0...v0.24.1
[0.24.0]: https://github.com/laqpay/laqpay/compare/v0.23.0...v0.24.0
[0.23.0]: https://github.com/laqpay/laqpay/compare/v0.22.0...v0.23.0
[0.22.0]: https://github.com/laqpay/laqpay/compare/v0.21.1...v0.22.0
[0.21.1]: https://github.com/laqpay/laqpay/compare/v0.21.0...v0.21.1
[0.21.0]: https://github.com/laqpay/laqpay/compare/v0.20.4...v0.21.0
[0.20.4]: https://github.com/laqpay/laqpay/compare/v0.20.3...v0.20.4
[0.20.3]: https://github.com/laqpay/laqpay/compare/v0.20.2...v0.20.3
[0.20.2]: https://github.com/laqpay/laqpay/compare/v0.20.1...v0.20.2
[0.20.1]: https://github.com/laqpay/laqpay/compare/v0.20.0...v0.20.1
[0.20.0]: https://github.com/laqpay/laqpay/compare/v0.19.1...v0.20.0
[0.19.1]: https://github.com/laqpay/laqpay/compare/v0.19.0...v0.19.1
[0.19.0]: https://github.com/laqpay/laqpay/commit/dd924e1f2de8fab945e05b3245dbeabf267f2910
