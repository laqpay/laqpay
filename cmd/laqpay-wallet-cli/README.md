# LAQ Wallet CLI Documentation

Laqpay command line interface

The CLI command APIs can be used directly from a Go application, see [Laqpay CLI Godoc](https://godoc.org/github.com/laqpay/laqpay/src/cli).
...
```bash
$ cd $GOPATH/src/github.com/laqpay/laqpay/cmd/laqpay-wallet-cli
$ ./install.sh
```
...
CLI will connect to laqpay node REST API address `http://127.0.0.1:6420` by default.
...
A username for authenticating requests to the laqpay node.
...
A password for authenticating requests to the laqpay node.
...
After the installation, you can run `laqpay-wallet-cli` to see the usage:

```
$ laqpay-wallet-cli

USAGE:
  laqpay-wallet-cli [command] [flags] [arguments...]

DESCRIPTION:
    The laqpay command line interface

COMMANDS:
  addPrivateKey         Add a private key to wallet
  addressBalance        Check the balance of specific addresses
  addressGen            Generate laqpay or bitcoin addresses
  ...
  richlist              Get laqpay richlist
  send                  Send laqpay from a wallet or an address to a recipient address
  showConfig            Show cli configuration
  showSeed              Show wallet seed and seed passphrase
  status                Check the status of current Laqpay node
  transaction           Show detail info of specific transaction
  verifyAddress         Verify a laqpay address
  verifyTransaction     Verify if the specific transaction is spendable
  version               List the current version of Laqpay components
  ...
  walletHistory         Display the transaction history of specific wallet. Requires laqpay node rpc.
  walletKeyExport       Export a specific key from an HD wallet
  walletOutputs         Display outputs of specific wallet

FLAGS:
  -h, --help      help for laqpay-wallet-cli
      --version   version for laqpay-wallet-cli

Use "laqpay-wallet-cli [command] --help" for more information about a command.
...
ENVIRONMENT VARIABLES:
    ...
    COIN: Name of the coin. Default "laqpay"
    DATA_DIR: Directory where everything is stored. Default "$HOME/.$COIN/"
```

### Add Private Key
Add a private key to a laqpay wallet.  Wallet type must be "collection".

```bash
$ laqpay-wallet-cli addPrivateKey [wallet] [private key]
```
...
#### Example
```bash
$ laqpay-wallet-cli addPrivateKey $WALLET_FILE $PRIVATE_KEY
```
...
```bash
$ laqpay-wallet-cli addressBalance [addresses]
```

#### Example
```bash
$ laqpay-wallet-cli addressBalance 2iVtHS5ye99Km5PonsB42No3pQRGEURmxyc 2GgFvqoyk9RjwVzj8tqfcXVXB4orBwoc9qv
```
<details>
...
### Generate addresses
Generate laqpay or bitcoin addresses.

```bash
$ laqpay-wallet-cli addressGen [flags]
```

```
FLAGS:
  -c, --coin string    Coin type. Must be laqpay or bitcoin. If bitcoin, secret keys are in Wallet Import Format instead of hex. (default "laqpay")
...
#### Examples
##### Generate `n` number of laqpay addresses
```bash
$ laqpay-wallet-cli addressGen --num 2
```
...
```bash
$ laqpay-wallet-cli addressGen --num 2 --coin bitcoin
```
...
```bash
$ laqpay-wallet-cli addressGen --num 2 --hide-secrets
```
...
```bash
$ laqpay-wallet-cli addressGen --num 2 --mode addresses
```
...
```bash
$ laqpay-wallet-cli addressGen --num 2 --seed "my super secret seed"
```
...
```bash
laqpay-wallet-cli addressGen --num 2 --hex
```
...
```bash
laqpay-wallet-cli fiberAddressGen [flags]
```
...
#### Examples
```bash
laqpay-wallet-cli fiberAddressGen
```
...
```bash
$ laqpay-wallet-cli addressOutputs [address list]
```

#### Example
```bash
laqpay-wallet-cli addressOutputs tWPDM36ex9zLjJw1aPMfYTVPbYgkL2Xp9V 29fDBQuJs2MDLymJsjyWH6rDjsyv995SrGU
```
...
```bash
$ laqpay-wallet-cli blocks [starting block or single block seq] [ending block seq]
```

#### Example
```bash
$ laqpay-wallet-cli blocks 41 42
```
...
Checks if the given database file contains valid laqpay blockchain data
If no argument is given, the default `data.db` in `$HOME/.$COIN/` will be checked.

```bash
$ laqpay-wallet-cli checkdb [db path]
```

#### Example
```bash
$ laqpay-wallet-cli checkdb $DB_PATH
```
...
```bash
$ laqpay-wallet-cli createRawTransaction [wallet] [to address] [amount] [flags]
```
...
```bash
$ laqpay-wallet-cli createRawTransaction $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS
```

##### Sending to a specific change address

```bash
$ laqpay-wallet-cli createRawTransaction $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS -c $CHANGE_ADDRESS
```
...
```bash
$ laqpay-wallet-cli createRawTransaction $WALLET_FILE -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
EOF
$ laqpay-wallet-cli createRawTransaction $WALLET_FILE -a $FROM_ADDRESS --csv $CSV_FILE
```
...
```bash
$ laqpay-wallet-cli createRawTransaction $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS --json
```
...
```bash
$ laqpay-wallet-cli decodeRawTransaction [raw transaction]
```

Decode a raw laqpay transaction.

#### Example

```bash
laqpay-wallet-cli decodeRawTransaction 
```
...
Encode JSON Laqpay transaction.

```bash
$ laqpay-wallet-cli encodeJsonTransaction [file path or -]
```
...
$ echo '  {
   ...
   }' | laqpay-wallet-cli encodeJsonTransaction -
```
...
$ laqpay-wallet-cli encodeJsonTransaction $FILEPATH
```
...
$ laqpay-wallet-cli encodeJsonTransaction -f $FILEPATH
```
...
$ laqpay-wallet-cli encodeJsonTransaction --json $FILEPATH
```
...
Broadcast a raw laqpay transaction.
Output is the transaction id.

```bash
$ laqpay-wallet-cli broadcastTransaction [raw transaction]
```

```bash
$ laqpay-wallet-cli broadcastTransaction 
```
<details>
 <summary>View Output</summary>
...
### Create a wallet
Create a new Laqpay wallet.

```bash
$ laqpay-wallet-cli walletCreate [wallet] [flags]
```
...
FLAGS:
  ...
  -c, --coin string              Wallet address coin type (options: laqpay, bitcoin) (default "laqpay")
...
Creates a deterministic wallet using the [Laqpay deterministic address generator](https://github.com/laqpay/laqpay/wiki/Deterministic-Keypair-Generation-Method).
Alternatively, you can create a `bip44` type wallet.

```bash
$ laqpay-wallet-cli walletCreate $WALLET_FILE -t deterministic
```
...
```bash
$ laqpay-wallet-cli walletCreate $WALLET_FILE -r
```
...
```bash
$ laqpay-wallet-cli walletCreate $WALLET_FILE -m
```
...
```bash
$ laqpay-wallet-cli walletCreate $WALLET_FILE -s "this is the super secret seed everyone needs but does not have"
```
...
```bash
$ laqpay-wallet-cli walletCreate $WALLET_FILE -n 2
```
...
```bash
$ laqpay-wallet-cli walletCreate $WALLET_FILE -l "cli wallet"
```
...
```bash
$ laqpay-wallet-cli walletCreate $WALLET_FILE -t collection
```
...
```bash
$ laqpay-wallet-cli walletCreate $WALLET_FILE -t bip44
```
...
```bash
$ laqpay-wallet-cli walletCreate $WALLET_FILE -t xpub --xpub xpub6FHa3pjLCk84BayeJxFW2SP4XRrFd1JYnxeLeU8EqN3vDfZmbqBqaGJAyiLjTAwm6ZLRQUMv1ZACTj37sR62cfN7fe5JnJ7dh8zL4fiyLHV
```
...
Add new addresses to a laqpay wallet.

```bash
$ laqpay-wallet-cli walletAddAddresses [wallet] [flags]
```
...
```bash
$ laqpay-wallet-cli walletAddAddresses $WALLET_FILE
```
...
##### Add `n` addresses
```bash
$ laqpay-wallet-cli walletAddAddresses $WALLET_FILE -n 2
```
...
```bash
$ laqpay-wallet-cli walletAddAddresses $WALLET_FILE --json
```
...
```bash
$ laqpay-wallet-cli walletKeyExport [wallet] [flags]
```
...
```bash
$ laqpay-wallet-cli walletKeyExport mywallet.wlt -k xpub -p "0/0"
```
...
```bash
$ laqpay-wallet-cli walletKeyExport mywallet.wlt -k xprv -p "0/1"
```
...
```bash
$ laqpay-wallet-cli walletKeyExport mywallet.wlt -k pub -p "0/0/5"
```
...
```bash
$ laqpay-wallet-cli walletKeyExport mywallet.wlt -k prv -p "0/1/5"
```
...
```bash
$ laqpay-wallet-cli walletKeyExport mywallet.wlt -k xpub -p "2"
```
...
```bash
$ laqpay-wallet-cli encryptWallet [wallet] [flags]
```
...
#### Encrypt wallet
```bash
$ laqpay-wallet-cli encryptWallet $WALLET_FILE -p test
```
...
```bash
$ laqpay-wallet-cli encryptWallet $WALLET_FILE -x sha256-xor -p test
```
...
```bash
$ laqpay-wallet-cli decryptWallet [wallet] [flags]
```
...
### Example
```bash
$ laqpay-wallet-cli decryptWallet $WALLET_FILE -p test
```
...
### Last blocks
Show the last `n` laqpay blocks.
By default the last block is shown.

```bash
$  laqpay-wallet-cli lastBlocks [numberOfBlocks]
```

#### Examples
##### Get the last block
```bash
$ laqpay-wallet-cli lastBlocks
```
...
```bash
$ laqpay-wallet-cli lastBlocks 3
```
...
List addresses in a laqpay wallet.

```bash
$ laqpay-wallet-cli listAddresses [wallet]
```

#### Example

```bash
$ laqpay-wallet-cli listAddresses $WALLET_FILE
```
...
### List wallets
List wallets in the Laqpay wallet directory (`$DATA_DIR/wallets`) or in a specific directory.

```bash
$ laqpay-wallet-cli listWallets [directory]
```
...
```json
{
    "directory": "/home/foo/.laqpay/wallets",
    "wallets": [
        {
...
```json
{
    "directory": "/home/foo/github.com/laqpay",
    "wallets": [
        {
            "name": "2018_02_04_45bc.wlt",
            "label": "Your Wallet",
...
```bash
$ laqpay-wallet-cli richlist [top N addresses] [include distribution addresses]
```

#### Example
```bash
$ laqpay-wallet-cli richlist 5 false
```
...
### Send
Make a laqpay transaction.

```bash
$ laqpay-wallet-cli send [wallet] [to address] [amount] [flags]
```
...
```bash
$ laqpay-wallet-cli send $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending from a specific address in a wallet
```bash
$ laqpay-wallet-cli send $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRRESS
```

##### Sending change to a specific change address
```bash
$ laqpay-wallet-cli send $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS -c $CHANGE_ADDRESS
```

##### Sending to multiple addresses
```bash
$ laqpay-wallet-cli send $WALLET_FILE -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
yExu4fryscnahAEMKa7XV4Wc1mY188KvGw,0.3
EOF
$ laqpay-wallet-cli send $WALLET_FILE -a $FROM_ADDRESS --csv $CSV_FILE
```
...
##### Generate a JSON output
```bash
$ laqpay-wallet-cli send $WALLET_FILE -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ laqpay-wallet-cli showSeed [wallet] [flags]
```
...
```bash
$ laqpay-wallet-cli showSeed $WALLET_FILE
```
...
```bash
$ laqpay-wallet-cli showSeed $WALLET_FILE -j
```
...
```json
{
    "data_directory": "/home/user/.laqpay",
    "coin": "laqpay",
...
```bash
$ laqpay-wallet-cli transaction [transaction id]
```

#### Example
```bash
$ laqpay-wallet-cli transaction 824d421a25f81aa7565d042a54b3e1e8fdc58bed4eefe8f8a90748da6d77d135
```
...
```bash
$ laqpay-wallet-cli addressTransactions [addr1 addr2 addr3]
```

#### Example
#### Single Address
```bash
$ laqpay-wallet-cli addressTransactions 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
#### Multiple Address
```bash
$ laqpay-wallet-cli addressTransactions 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda 3vbfHxPzMuyFJvgHdAoqmFnyg6k8HiLyxd
```
...
### Verify address
Verify whether a given address is a valid laqpay addres or not.

```bash
$  laqpay-wallet-cli verifyAddress [laqpay address]
```
...
```bash
$ laqpay-wallet-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
###### Invalid checksum
```bash
$ laqpay-wallet-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsdx
```
...
```bash
$ laqpay-wallet-cli verifyAddress 21YPg
```
...
Check the wallet a laqpay wallet.

```bash
$ laqpay-wallet-cli walletBalance [wallet]
```

#### Example
##### Balance of default wallet
```bash
$ laqpay-wallet-cli walletBalance
```
...
```bash
$ laqpay-wallet-cli walletBalance 2018_04_01_198c.wlt
```
*OR*

```bash
$ laqpay-wallet-cli walletBalance ~/.laqpay/wallets/2018_04_01_198c.wlt
```
...
```bash
$ laqpay-wallet-cli walletHistory [wallet]
```

#### Example

```bash
$ laqpay-wallet-cli walletHistory $WALLET_FILE
```
...
```bash
$ laqpay-wallet-cli walletOutputs [wallet]
```
...
```bash
$ laqpay-wallet-cli richlist [top N addresses (20 default)] [include distribution addresses (false default)]
```

#### Example
##### Without distribution addresses
```bash
$ laqpay-wallet-cli richlist 2
```
<details>
 <summary>View Output</summary>
...
```bash
$ laqpay-wallet-cli richlist 2 true
```
...
```bash
$ laqpay-wallet-cli addresscount
```
...
### CLI version
Get version of current laqpay cli.

```bash
$ laqpay-wallet-cli version [flags]
```
...
##### JSON output
```bash
$ laqpay-wallet-cli version --json
```
...
```bash
$ laqpay-wallet-cli distributeGenesis [genesis address secret key]
```
...
```bash
$ laqpay-wallet-cli distributeGenesis 87b2ae65a7475481f73288b617242261e2a451815e1c6a5478862dbb95e23e3e
```
