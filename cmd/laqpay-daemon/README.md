...
```
» go run cmd/laqpay-daemon/laqpay-daemon.go --help
...
Usage:
  ...
  -data-dir string
    	directory to store app data (defaults to ~/.laqpay) (default "$HOME/.laqpay")
  -db-path string
    	path of database file (defaults to ~/.laqpay/data.db)
  -db-read-only
  ...
  -peerlist-url string
    	with -download-peerlist=true, download a peers.txt file from this url (default "https://api.laqpay.com/network/peers")
  -port int
  ...
  -storage-dir string
    	location of the storage data files. Defaults to ~/.laqpay/data/
  ...
  -wallet-dir string
    	location of the wallet files. Defaults to ~/.laqpay/wallet/
  -web-interface
  ...
  -web-interface-cert string
    	laqpayd.cert file for web interface HTTPS. If not provided, will autogenerate or use laqpayd.cert in --data-dir
  -web-interface-https
    	enable HTTPS for web interface
  -web-interface-key string
    	laqpayd.key file for web interface HTTPS. If not provided, will autogenerate or use laqpayd.key in --data-dir
...
Read more about API sets here: https://github.com/laqpay/laqpay/blob/develop/src/api/README.md#api-sets
...
```sh
$ go run cmd/laqpay-daemon/laqpay-daemon.go \
...
go run cmd/laqpay-daemon/laqpay-daemon.go \
  --web-interface-https \
  --web-interface-key=/var/local/laqpayd.key \
  --web-interface-cert=/var/local/laqpayd.cert \
  --enable-api-sets=READ,TXN
```
...
On Linux and MacOS, this folder defaults to `$HOME/.laqpay` (`~/.laqpay`).
On Windows release builds, this folder defaults to `%HOMEPATH%\.laqpay` (`C:\Users\{user}\.laqpay`).
On Windows development builds, this folder defaults to `C:\.laqpay`. *(Note: this is a bug and will change in the future)*
...
Read more about `pprof`: https://github.com/laqpay/laqpay/wiki/Profiling-with-pprof
...
The size of a transaction is the length of its byte representation in the [Laqpay binary encoding format](https://github.com/laqpay/laqpay/wiki/Laqpay-Binary-Encoding-Format).
...
The peers in the hardcoded default peer list are maintained by Laqpay and are kept up to date with current
...
The URL of the remote peer list bootstrap file. Defaults to https://api.laqpay.com/network/peers.
...
The certificate file for the HTTPS REST API. If not provided and HTTPS is enabled, the cert defaults to a file named `laqpayd.cert`
...
The key file for the HTTPS REST API. If not provided and HTTPS is enabled, the cert defaults to a file named `laqpayd.key`
in the `data-dir`. If this file does not exist, it will be autogenerated.
