...
```bash
$ cd $GOPATH/src/github.com/laqpay/laqpay/cmd/cli
$ ./install.sh
```
...
After the installation, you can run `laqpay-cli` to see the usage:

```
$ laqpay-cli

USAGE:
  laqpay-cli [command] [flags] [arguments...]
...
FLAGS:
  -h, --help      help for laqpay-cli
      --version   version for laqpay-cli

Use "laqpay-cli [command] --help" for more information about a command.
...
```bash
$ laqpay-cli addPrivateKey [wallet] [private key]
```
...
#### Example
```bash
$ laqpay-cli addPrivateKey $WALLET_FILE $PRIVATE_KEY
```
...
```bash
$ laqpay-cli addressBalance [addresses]
```

#### Example
```bash
$ laqpay-cli addressBalance 2iVtHS5ye99Km5PonsB42No3pQRGEURmxyc 2GgFvqoyk9RjwVzj8tqfcXVXB4orBwoc9qv
```
<details>
...
```bash
$ laqpay-cli addressGen --num 2 --coin bitcoin
```
...
```bash
$ laqpay-cli addressGen --num 2 --hide-secrets
```
...
```bash
$ laqpay-cli addressGen --num 2 --mode addresses
```
...
```bash
$ laqpay-cli addressGen --num 2 --seed "my super secret seed"
```
...
```bash
laqpay-cli addressGen --num 2 --hex
```
...
```bash
laqpay-cli fiberAddressGen [flags]
```
...
```bash
$ laqpay-cli addressOutputs [address list]
```

#### Example
```bash
laqpay-cli addressOutputs tWPDM36ex9zLjJw1aPMfYTVPbYgkL2Xp9V 29fDBQuJs2MDLymJsjyWH6rDjsyv995SrGU
```
...
```bash
$ laqpay-cli blocks [starting block or single block seq] [ending block seq]
```

#### Example
```bash
$ laqpay-cli blocks 41 42
```
...
```bash
$ laqpay-cli checkdb [db path]
```

#### Example
```bash
$ laqpay-cli checkdb $DB_PATH
```
...
```bash
$ laqpay-cli createRawTransaction [wallet] [to address] [amount] [flags]
```
...
```bash
$ laqpay-cli createRawTransaction $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS
```

##### Sending to a specific change address

```bash
$ laqpay-cli createRawTransaction $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS -c $CHANGE_ADDRESS
```
...
```bash
$ laqpay-cli createRawTransaction $WALLET_FILE -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
EOF
$ laqpay-cli createRawTransaction $WALLET_FILE -a $FROM_ADDRESS --csv $CSV_FILE
```
...
```bash
$ laqpay-cli createRawTransaction $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS --json
```
...
```bash
$ laqpay-cli decodeRawTransaction [raw transaction]
```
...
```bash
laqpay-cli decodeRawTransaction 
```
...
```bash
$ laqpay-cli encodeJsonTransaction [file path or -]
```
...
$ echo '  {
   ...
   }' | laqpay-cli encodeJsonTransaction -
```
...
$ laqpay-cli encodeJsonTransaction $FILEPATH
```
...
$ laqpay-cli encodeJsonTransaction -f $FILEPATH
```
...
$ laqpay-cli encodeJsonTransaction --json $FILEPATH
```
...
```bash
$ laqpay-cli broadcastTransaction [raw transaction]
```

```bash
$ laqpay-cli broadcastTransaction 
```
<details>
 <summary>View Output</summary>
...
```bash
$ laqpay-cli walletCreate [wallet] [flags]
```
...
```bash
$ laqpay-cli walletCreate $WALLET_FILE -t deterministic
```
...
```bash
$ laqpay-cli walletCreate $WALLET_FILE -r
```
...
```bash
$ laqpay-cli walletCreate $WALLET_FILE -m
```
...
```bash
$ laqpay-cli walletCreate $WALLET_FILE -s "this is the super secret seed everyone needs but does not have"
```
...
```bash
$ laqpay-cli walletCreate $WALLET_FILE -n 2
```
...
```bash
$ laqpay-cli walletCreate $WALLET_FILE -l "cli wallet"
```
...
```bash
$ laqpay-cli walletCreate $WALLET_FILE -t collection
```
...
```bash
$ laqpay-cli walletCreate $WALLET_FILE -t bip44
```
...
```bash
$ laqpay-cli walletCreate $WALLET_FILE -t xpub --xpub xpub6FHa3pjLCk84BayeJxFW2SP4XRrFd1JYnxeLeU8EqN3vDfZmbqBqaGJAyiLjTAwm6ZLRQUMv1ZACTj37sR62cfN7fe5JnJ7dh8zL4fiyLHV
```
...
```bash
$ laqpay-cli walletAddAddresses [wallet] [flags]
```
...
```bash
$ laqpay-cli walletAddAddresses $WALLET_FILE
```
...
##### Add `n` addresses
```bash
$ laqpay-cli walletAddAddresses $WALLET_FILE -n 2
```
...
```bash
$ laqpay-cli walletAddAddresses $WALLET_FILE --json
```
...
```bash
$ laqpay-cli walletKeyExport [wallet] [flags]
```
...
```bash
$ laqpay-cli walletKeyExport mywallet.wlt -k xpub -p "0/0"
```
...
```bash
$ laqpay-cli walletKeyExport mywallet.wlt -k xprv -p "0/1"
```
...
```bash
$ laqpay-cli walletKeyExport mywallet.wlt -k pub -p "0/0/5"
```
...
```bash
$ laqpay-cli walletKeyExport mywallet.wlt -k prv -p "0/1/5"
```
...
```bash
$ laqpay-cli walletKeyExport mywallet.wlt -k xpub -p "2"
```
...
```bash
$ laqpay-cli encryptWallet [wallet] [flags]
```
...
#### Encrypt wallet
```bash
$ laqpay-cli encryptWallet $WALLET_FILE -p test
```
...
```bash
$ laqpay-cli encryptWallet $WALLET_FILE -x sha256-xor -p test
```
...
```bash
$ laqpay-cli decryptWallet [wallet] [flags]
```
...
### Example
```bash
$ laqpay-cli decryptWallet $WALLET_FILE -p test
```
...
```bash
$  laqpay-cli lastBlocks [numberOfBlocks]
```

#### Examples
##### Get the last block
```bash
$ laqpay-cli lastBlocks
```
...
```bash
$ laqpay-cli listAddresses [wallet]
```

#### Example

```bash
$ laqpay-cli listAddresses $WALLET_FILE
```
...
```bash
$ laqpay-cli listWallets [directory]
```
...
```bash
$ laqpay-cli richlist [top N addresses] [include distribution addresses]
```

#### Example
```bash
$ laqpay-cli richlist 5 false
```
...
```bash
$ laqpay-cli send [wallet] [to address] [amount] [flags]
```
...
```bash
$ laqpay-cli send $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT
```

##### Sending from a specific address in a wallet
```bash
$ laqpay-cli send $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRRESS
```

##### Sending change to a specific change address
```bash
$ laqpay-cli send $WALLET_FILE $RECIPIENT_ADDRESS $AMOUNT -a $FROM_ADDRESS -c $CHANGE_ADDRESS
```

##### Sending to multiple addresses
```bash
$ laqpay-cli send $WALLET_FILE -a $FROM_ADDRESS -m '[{"addr":"$ADDR1", "coins": "$AMT1"}, {"addr":"$ADDR2", "coins": "$AMT2"}]'
```
...
yExu4fryscnahAEMKa7XV4Wc1mY188KvGw,0.3
EOF
$ laqpay-cli send $WALLET_FILE -a $FROM_ADDRESS --csv $CSV_FILE
```
...
##### Generate a JSON output
```bash
$ laqpay-cli send $WALLET_FILE -a $FROM_ADDRESS --json $RECIPIENT_ADDRESS $AMOUNT
```
...
```bash
$ laqpay-cli showSeed [wallet] [flags]
```
...
```bash
$ laqpay-cli showSeed $WALLET_FILE
```
...
```bash
$ laqpay-cli showSeed $WALLET_FILE -j
```
...
```bash
$ laqpay-cli transaction [transaction id]
```

#### Example
```bash
$ laqpay-cli transaction 824d421a25f81aa7565d042a54b3e1e8fdc58bed4eefe8f8a90748da6d77d135
```
...
```bash
$ laqpay-cli addressTransactions [addr1 addr2 addr3]
```

#### Example
#### Single Address
```bash
$ laqpay-cli addressTransactions 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
#### Multiple Address
```bash
$ laqpay-cli addressTransactions 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda 3vbfHxPzMuyFJvgHdAoqmFnyg6k8HiLyxd
```
...
```bash
$  laqpay-cli verifyAddress [laqpay address]
```
...
```bash
$ laqpay-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsda
```
...
###### Invalid checksum
```bash
$ laqpay-cli verifyAddress 21YPgFwkLxQ1e9JTCZ43G7JUyCaGRGqAsdx
```
...
```bash
$ laqpay-cli verifyAddress 21YPg
```
...
```bash
$ laqpay-cli walletBalance [wallet]
```

#### Example
##### Balance of default wallet
```bash
$ laqpay-cli walletBalance
```
...
```bash
$ laqpay-cli walletBalance 2018_04_01_198c.wlt
```
*OR*

```bash
$ laqpay-cli walletBalance ~/.laqpay/wallets/2018_04_01_198c.wlt
```
...
```bash
$ laqpay-cli walletHistory [wallet]
```

#### Example

```bash
$ laqpay-cli walletHistory $WALLET_FILE
```
...
```bash
$ laqpay-cli walletOutputs [wallet]
```
...
```bash
$ laqpay-cli richlist [top N addresses (20 default)] [include distribution addresses (false default)]
```

#### Example
##### Without distribution addresses
```bash
$ laqpay-cli richlist 2
```
<details>
 <summary>View Output</summary>
...
```bash
$ laqpay-cli distributeGenesis [genesis address secret key]
```
...
```bash
$ laqpay-cli distributeGenesis 87b2ae65a7475481f73288b617242261e2a451815e1c6a5478862dbb95e23e3e
```
