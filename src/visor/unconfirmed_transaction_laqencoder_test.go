...
func testLaqencoderUnconfirmedTransaction(t *testing.T, obj *UnconfirmedTransaction) {
	...
	// Encode
	data2, err := encodeUnconfirmedTransaction(obj)
	if err != nil {
		t.Fatalf("encodeUnconfirmedTransaction failed: %v", err)
	}
	if uint64(len(data2)) != n2 {
		t.Fatal("encodeUnconfirmedTransaction produced bytes of unexpected length")
	}
	...
	if err := encodeUnconfirmedTransactionToBuffer(data3, obj); err != nil {
		t.Fatalf("encodeUnconfirmedTransactionToBuffer failed: %v", err)
	}
	...
	if n, err := decodeUnconfirmedTransaction(data2, &obj3); err != nil {
	...
	var obj4 UnconfirmedTransaction
	n, err := decodeUnconfirmedTransaction(data3, &obj4)
	if err != nil {
		t.Fatalf("decodeUnconfirmedTransaction failed: %v", err)
	}
	...
	if hasOmitEmptyField(&obj4) && omitEmptyLen(&obj4) == 0 {
		...
		if n != n2+4 {
			t.Fatalf("decodeUnconfirmedTransaction bytes read length should be %d, is %d", n2+4, n)
		}
	} else {
		if n != n2 {
			t.Fatalf("decodeUnconfirmedTransaction bytes read length should be %d, is %d", n2, n)
		}
	}
	...
	// DecodeExact
	var obj5 UnconfirmedTransaction
	if err := decodeUnconfirmedTransactionExact(data2, &obj5); err != nil {
		t.Fatalf("decodeUnconfirmedTransaction failed: %v", err)
	}
	...
	if !hasOmitEmptyField(&obj3) || omitEmptyLen(&obj3) > 0 {
		...
		if n, err := decodeUnconfirmedTransaction(data4, &obj3); err != nil {
			t.Fatalf("decodeUnconfirmedTransaction failed: %v", err)
		} else if n != uint64(len(data2)) {
			t.Fatalf("decodeUnconfirmedTransaction bytes read length should be %d, is %d", len(data2), n)
		}
...
func TestLaqencoderUnconfirmedTransaction(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testLaqencoderUnconfirmedTransaction(t, tc.obj)
		})
	}
}
...
func decodeUnconfirmedTransactionExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj UnconfirmedTransaction
	if _, err := decodeUnconfirmedTransaction(buf, &obj); err == nil {
...
func decodeUnconfirmedTransactionExactExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj UnconfirmedTransaction
	if err := decodeUnconfirmedTransactionExact(buf, &obj); err == nil {
		t.Fatal("decodeUnconfirmedTransactionExact: expected error, got nil")
	} else if err != expectedErr {
		t.Fatalf("decodeUnconfirmedTransactionExact: expected error %q, got %q", expectedErr, err)
	}
}

func testLaqencoderUnconfirmedTransactionDecodeErrors(t *testing.T, k int, tag string, obj *UnconfirmedTransaction) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
	...
	buf, err := encodeUnconfirmedTransaction(obj)
	if err != nil {
		t.Fatalf("encodeUnconfirmedTransaction failed: %v", err)
	}
	...
	if hasOmitEmptyField(obj) && numEncodableFields(obj) > 1 {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow nil", k, tag), func(t *testing.T) {
			decodeUnconfirmedTransactionExactExpectError(t, nil, encoder.ErrBufferUnderflow)
		})
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow bytes=%d", k, tag, i), func(t *testing.T) {
			decodeUnconfirmedTransactionExactExpectError(t, buf[:i], encoder.ErrBufferUnderflow)
		})
	}
	...
	t.Run(fmt.Sprintf("%d %s exact buffer remaining bytes", k, tag), func(t *testing.T) {
		decodeUnconfirmedTransactionExactExpectError(t, buf, encoder.ErrRemainingBytes)
	})
}

func TestLaqencoderUnconfirmedTransactionDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoderUnconfirmedTransactionDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderUnconfirmedTransactionDecodeErrors(t, i, "full", fullObj)
	}
}
