...
func testLaqencoderSigWrapper(t *testing.T, obj *sigWrapper) {
...
func TestLaqencoderSigWrapper(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testLaqencoderSigWrapper(t, tc.obj)
		})
	}
}
...
func testLaqencoderSigWrapperDecodeErrors(t *testing.T, k int, tag string, obj *sigWrapper) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestLaqencoderSigWrapperDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoderSigWrapperDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderSigWrapperDecodeErrors(t, i, "full", fullObj)
	}
}
