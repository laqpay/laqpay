...
// hashesWrapper wraps []cipher.SHA256 so it can be used by laqencoder
type hashesWrapper struct {
	Hashes []cipher.SHA256
}

// sigWrapper wraps cipher.Sig in struct so it can be used by laqencoder
type sigWrapper struct {
	Sig cipher.Sig
}

// hashPairsWrapper wraps []coin.HashPair so it can be used by laqencoder
