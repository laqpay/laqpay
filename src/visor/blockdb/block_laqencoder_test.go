...
func testLaqencoderBlock(t *testing.T, obj *coin.Block) {
...
func TestLaqencoderBlock(t *testing.T) {
...
func testLaqencoderBlockDecodeErrors(t *testing.T, k int, tag string, obj *coin.Block) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestLaqencoderBlockDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoderBlockDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderBlockDecodeErrors(t, i, "full", fullObj)
	}
}
