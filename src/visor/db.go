...
// VerifyDBLaqencoderSafe verifies that the laqencoder generated code has the same result as the encoder
// for all data in the blockchain
func VerifyDBLaqencoderSafe(db *dbutil.DB, quit <-chan struct{}) error {
	return db.View("VerifyDBLaqencoderSafe", func(tx *dbutil.Tx) error {
		return verifyDBLaqencoderSafe(tx, quit)
	})
}

func verifyDBLaqencoderSafe(tx *dbutil.Tx, quit <-chan struct{}) error {
	if quit == nil {
		quit = make(chan struct{})
	}

	// blockdb
	if err := blockdb.VerifyDBLaqencoderSafe(tx, quit); err != nil {
	...
	// historydb
	if err := historydb.VerifyDBLaqencoderSafe(tx, quit); err != nil {
