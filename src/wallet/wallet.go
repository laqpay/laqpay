...
type Options struct {
	...
	Coin           CoinType        // coin type: laqpay, bitcoin, etc. Refers to which pubkey2addr method is used.
...
func newWallet(wltName string, opts Options, tf TransactionsFinder) (Wallet, error) {
	...
	case WalletTypeDeterministic, WalletTypeBip44, WalletTypeXPub:
		...
		if opts.ScanN != 0 && coin != CoinTypeLaqpay {
			return nil, errors.New("Wallet scanning is only supported for Laqpay address wallets")
		}
...
type Wallet interface {
	...
	GetLaqpayAddresses() ([]cipher.Address, error)
	...
	GenerateLaqpayAddresses(num uint64) ([]cipher.Address, error)
...
func Load(filename string) (Wallet, error) {
	...
	// Make sure "laq", "btc" normalize to "laqpay", "bitcoin"
