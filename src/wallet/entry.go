...
// LaqpayAddress returns the Laqpay address of an entry. Panics if Address is not a Laqpay address
func (we Entry) LaqpayAddress() cipher.Address {
	return we.Address.(cipher.Address)
}

// BitcoinAddress returns the Laqpay address of an entry. Panics if Address is not a Bitcoin address
...
func (entries Entries) getLaqpayAddresses() []cipher.Address {
	addrs := make([]cipher.Address, len(entries))
	for i, e := range entries {
		addrs[i] = e.LaqpayAddress()
	}
	return addrs
}
