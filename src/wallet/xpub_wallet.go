...
func (w *XPubWallet) GetLaqpayAddresses() ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeLaqpay {
		return nil, errors.New("XPubWallet coin type is not laqpay")
	}
...
func (w *XPubWallet) GenerateLaqpayAddresses(num uint64) ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeLaqpay {
