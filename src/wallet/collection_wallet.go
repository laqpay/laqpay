...
// GenerateLaqpayAddresses is a no-op for "collection" wallets
func (w *CollectionWallet) GenerateLaqpayAddresses(num uint64) ([]cipher.Address, error) {
	return nil, NewError(errors.New("A collection wallet does not implement GenerateLaqpayAddresses"))
}
...
func (w *CollectionWallet) GetLaqpayAddresses() ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeLaqpay {
		return nil, errors.New("CollectionWallet coin type is not laqpay")
	}
...
func (w *CollectionWallet) AddEntry(e Entry) error {
	...
	for _, entry := range w.Entries {
		if e.LaqpayAddress() == entry.LaqpayAddress() {
