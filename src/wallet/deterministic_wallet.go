...
func (w *DeterministicWallet) GetLaqpayAddresses() ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeLaqpay {
		return nil, errors.New("DeterministicWallet coin type is not laqpay")
	}

	return w.Entries.getLaqpayAddresses(), nil
}
...
// GenerateLaqpayAddresses generates Laqpay addresses. If the wallet's coin type is not Laqpay, returns an error
func (w *DeterministicWallet) GenerateLaqpayAddresses(num uint64) ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeLaqpay {
	...
	laqAddrs := make([]cipher.Address, len(addrs))
	for i, a := range addrs {
		laqAddrs[i] = a.(cipher.Address)
	}
...
func (w *DeterministicWallet) ScanAddresses(scanN uint64, tf TransactionsFinder) error {
	...
	for {
		...
		addrs, err := w2.GenerateLaqpayAddresses(n)
		if err != nil {
			return err
		}
	...
	w2.reset()
	if _, err := w2.GenerateLaqpayAddresses(nExistingAddrs + nAddAddrs); err != nil {
		return err
	}
