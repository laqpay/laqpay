...
// Bip44Wallet manages keys using the original Laqpay deterministic
...
// GetLaqpayAddresses returns all Laqpay addresses in wallet. The wallet's coin type must be Laqpay.
func (w *Bip44Wallet) GetLaqpayAddresses() ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeLaqpay {
		return nil, errors.New("Bip44Wallet coin type is not laqpay")
	}

	return append(w.ExternalEntries.getLaqpayAddresses(), w.ChangeEntries.getLaqpayAddresses()...), nil
}
...
// GenerateLaqpayAddresses generates Laqpay addresses for the external chain, and appends them to the wallet's entries array.
// If the wallet's coin type is not Laqpay, returns an error
func (w *Bip44Wallet) GenerateLaqpayAddresses(num uint64) ([]cipher.Address, error) {
	if w.Meta.Coin() != CoinTypeLaqpay {
		return nil, errors.New("GenerateLaqpayAddresses called for non-laqpay wallet")
	}
	...
	return entries.getLaqpayAddresses(), nil
}
...
func scanAddressesBip32(generateEntries func(num uint64, childIdx uint32) (Entries, error), scanN uint64, tf TransactionsFinder, initialChildIdx uint32) (Entries, error) {
	...
	for {
		...
		addrs := entries.getLaqpayAddresses()
