...
see [Laqpay REST API Client Godoc](https://godoc.org/github.com/laqpay/laqpay/src/api#Client).
...
* `PROMETHEUS` - This is the `/api/v2/metrics` method exposing in Prometheus text format the default metrics for Laqpay node application
...
Parses and validates a Laqpay address. Returns the address version in the response.
...
```json
{
    "address": "/Users/user/.laqpay/wallets"
}
```
...
Args:
    ...
    bip44-coin: BIP44 coin type [optional, defaults to 8000 (laqpay's coin type), only valid if type is "bip44"]
...
See [`coin.Transaction.Serialize`](https://godoc.org/github.com/laqpay/laqpay/src/coin#Transaction.Serialize).
...
You must use a string for `"coins"` instead of an integer measured in "droplets" (the smallest unit of currency in Laqpay, 1/1000000 of a laqpay).
For example, if you sent 1 Laqpay with `/api/v1/spend` you would have specified the `coins` field as `1000000`.
