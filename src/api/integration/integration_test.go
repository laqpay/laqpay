...
/* Runs HTTP API tests against a running laqpay node

Set envvar LAQPAY_INTEGRATION_TESTS=1 to enable them
Set LAQPAY_NODE_HOST to the node's address (defaults to http://127.0.0.1:6420)
Set LAQPAY_INTEGRATION_TEST_MODE to either "stable" or "live" (defaults to "stable")
...
Make sure the laqpay node is running against the pinned blockchain data provided in this package's folder.
...
func nodeAddress() string {
	addr := os.Getenv("LAQPAY_NODE_HOST")
	if addr == "" {
...
func nodeUsername() string {
	return os.Getenv("LAQPAY_NODE_USERNAME")
}

func nodePassword() string {
	return os.Getenv("LAQPAY_NODE_PASSWORD")
}
