...
REST API to query a laqpay node's status.
*/
package cli
...
func NewCLI(cfg Config) (*cobra.Command, error) {
	...
	laqCLI.Version = Version
	laqCLI.SuggestionsMinimumDistance = 1
	laqCLI.AddCommand(commands...)

	laqCLI.SetHelpTemplate(helpTemplate)
	laqCLI.SetUsageTemplate(helpTemplate)
