# Laqpay desktop client

The Laqpay wallet ships with a web interface which can be ran from the browser and/or Electron.

The project contains both the source (src) and target (dist) files of this web interface.

## Prerequisites

The Laqpay web interface requires Node 8.10.0 or higher, together with NPM 5.6 or higher.
...
While event.laqpay.com is not working, we will have to run the purchase API locally.
