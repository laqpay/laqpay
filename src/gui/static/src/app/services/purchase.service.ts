...
export class PurchaseService {
  ...
  getConfig() {
    return this.get('config')
      .map((response: any) => ({
        enabled: true,
        laq_btc_exchange_rate: parseFloat(response.laq_btc_exchange_rate),
      }))
  ...
  generate(wallet: Wallet): Observable<PurchaseOrder> {
    return this.walletService.addAddress(wallet, 1).flatMap(address => {
      return this.post('bind', { laqaddr: address[0].address, coin_type: 'BTC' })
        .map(response => ({
  ...
  scan(address: string) {
    return this.get('status?laqaddr=' + address)
      .map((response: any) => {
