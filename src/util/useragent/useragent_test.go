...
func TestDataBuild(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "with remark",
			userAgent: "Laqpay:0.24.1(remark; foo)",
			data: Data{
				Coin:    "Laqpay",
				Version: "0.24.1",
				Remark:  "remark; foo",
...
func TestParse(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "too long",
			userAgent: fmt.Sprintf("laqpay:0.24.1[abcdefg](%s)", strings.Repeat("a", 245)),
			err:       ErrTooLong,
		},
		{
...
func TestDataJSON(t *testing.T) {
	...
	x, err = json.Marshal(d)
	require.NoError(t, err)
	require.Equal(t, `"laqpay:0.25.0"`, string(x))
	...
	x, err = json.Marshal(d)
	require.NoError(t, err)
	require.Equal(t, `"laqpay:0.25.0(foo; bar)"`, string(x))
	...
	err = json.Unmarshal([]byte(`"laqpay:0.25.0(<>)"`), &e)
...
func TestSanitize(t *testing.T) {
	...
	x := "Laqpay:0.25.0(foo; bar)"
	require.Equal(t, x, Sanitize(x))

	// Should not have anything stripped
	x = "Laqpay:0.25.1(foo; bar)"
	require.Equal(t, x, Sanitize(x))
}
