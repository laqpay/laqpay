// package laqpay implements laqpay main integration tests
package laqpay
...
var (
	// import path for github.com/laqpay/laqpay/cmd/laqpay
	ldflagsNameCmd string

	// import path for github.com/laqpay/laqpay/src/laqpay
	ldflagsNameLaqLib string
)
...
func TestMain(m *testing.M) {
	...
	ldflagsNameLaqLib = strings.TrimSpace(string(output))
...
func buildBinary(t *testing.T, version string) (string, func()) {
	...
	binaryName := fmt.Sprintf("%s-laqpay-pkg-%s.test", coin, version)
	binaryPath, err := filepath.Abs(binaryName)
	require.NoError(t, err)

	// coverpkgName will be like github.com/laqpay/laqpay
	...
	args := []string{
		"test", "-c",
		"-ldflags", fmt.Sprintf("-X %s.Version=%s -X %s.DBVerifyCheckpointVersion=%s", ldflagsNameCmd, version, ldflagsNameLaqLib, checkpointVersion),
		"-tags", "testrunmain",
		"-o", binaryPath,
