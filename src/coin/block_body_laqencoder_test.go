...
func testLaqencoderBlockBody(t *testing.T, obj *BlockBody) {
...
func TestLaqencoderBlockBody(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testLaqencoderBlockBody(t, tc.obj)
		})
	}
}
...
func testLaqencoderBlockBodyDecodeErrors(t *testing.T, k int, tag string, obj *BlockBody) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestLaqencoderBlockBodyDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoderBlockBodyDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderBlockBodyDecodeErrors(t, i, "full", fullObj)
	}
}
