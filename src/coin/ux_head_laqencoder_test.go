...
func testLaqencoderUxHead(t *testing.T, obj *UxHead) {
...
func TestLaqencoderUxHead(t *testing.T) {
...
func testLaqencoderUxHeadDecodeErrors(t *testing.T, k int, tag string, obj *UxHead) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestLaqencoderUxHeadDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoderUxHeadDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderUxHeadDecodeErrors(t, i, "full", fullObj)
	}
}
