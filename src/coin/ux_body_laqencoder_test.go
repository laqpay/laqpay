...
func testLaqencoderUxBody(t *testing.T, obj *UxBody) {
...
func TestLaqencoderUxBody(t *testing.T) {
...
func testLaqencoderUxBodyDecodeErrors(t *testing.T, k int, tag string, obj *UxBody) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestLaqencoderUxBodyDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoderUxBodyDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderUxBodyDecodeErrors(t, i, "full", fullObj)
	}
}
