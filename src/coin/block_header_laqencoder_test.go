...
func testLaqencoderBlockHeader(t *testing.T, obj *BlockHeader) {
...
func TestLaqencoderBlockHeader(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testLaqencoderBlockHeader(t, tc.obj)
		})
	}
}
...
func testLaqencoderBlockHeaderDecodeErrors(t *testing.T, k int, tag string, obj *BlockHeader) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestLaqencoderBlockHeaderDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoderBlockHeaderDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderBlockHeaderDecodeErrors(t, i, "full", fullObj)
	}
}
