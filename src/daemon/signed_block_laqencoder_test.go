...
func testLaqencoderSignedBlock(t *testing.T, obj *coin.SignedBlock) {
...
func TestLaqencoderSignedBlock(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testLaqencoderSignedBlock(t, tc.obj)
		})
	}
}
...
func testLaqencoderSignedBlockDecodeErrors(t *testing.T, k int, tag string, obj *coin.SignedBlock) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestLaqencoderSignedBlockDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoderSignedBlockDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderSignedBlockDecodeErrors(t, i, "full", fullObj)
	}
}
