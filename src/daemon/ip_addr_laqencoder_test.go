// Code generated by github.com/laqpay/laqencoder. DO NOT EDIT.
...
import (
	...
	"github.com/laqpay/encodertest"

	"github.com/laqpay/laqpay/src/cipher/encoder"
)

func newEmptyIPAddrForEncodeTest() *IPAddr {
	var obj IPAddr
	return &obj
}

func newRandomIPAddrForEncodeTest(t *testing.T, rand *mathrand.Rand) *IPAddr {
	var obj IPAddr
...
func newRandomZeroLenIPAddrForEncodeTest(t *testing.T, rand *mathrand.Rand) *IPAddr {
	var obj IPAddr
	err := encodertest.PopulateRandom(&obj, rand, encodertest.PopulateRandomOptions{
		MaxRandLen:    0,
		MinRandLen:    0,
...
func newRandomZeroLenNilIPAddrForEncodeTest(t *testing.T, rand *mathrand.Rand) *IPAddr {
	var obj IPAddr
	err := encodertest.PopulateRandom(&obj, rand, encodertest.PopulateRandomOptions{
		MaxRandLen:    0,
		MinRandLen:    0,
...
func testLaqencoderIPAddr(t *testing.T, obj *IPAddr) {
	...
	n1 := encoder.Size(obj)
	n2 := encodeSizeIPAddr(obj)

	if uint64(n1) != n2 {
		t.Fatalf("encoder.Size() != encodeSizeIPAddr() (%d != %d)", n1, n2)
	}
	...
	// Encode
	data2, err := encodeIPAddr(obj)
	if err != nil {
		t.Fatalf("encodeIPAddr failed: %v", err)
	}
	if uint64(len(data2)) != n2 {
		t.Fatal("encodeIPAddr produced bytes of unexpected length")
	}
	if len(data1) != len(data2) {
		t.Fatalf("len(encoder.Serialize()) != len(encodeIPAddr()) (%d != %d)", len(data1), len(data2))
	}

	// EncodeToBuffer
	data3 := make([]byte, n2+5)
	if err := encodeIPAddrToBuffer(data3, obj); err != nil {
		t.Fatalf("encodeIPAddrToBuffer failed: %v", err)
	}
	...
	// Decode
	var obj3 IPAddr
	if n, err := decodeIPAddr(data2, &obj3); err != nil {
		t.Fatalf("decodeIPAddr failed: %v", err)
	} else if n != uint64(len(data2)) {
		t.Fatalf("decodeIPAddr bytes read length should be %d, is %d", len(data2), n)
	}
	if !cmp.Equal(obj2, obj3, cmpopts.EquateEmpty(), encodertest.IgnoreAllUnexported()) {
		t.Fatal("encoder.DeserializeRaw() != decodeIPAddr()")
	}

	// Decode, excess buffer
	var obj4 IPAddr
	n, err := decodeIPAddr(data3, &obj4)
	if err != nil {
		t.Fatalf("decodeIPAddr failed: %v", err)
	}
	...
	if hasOmitEmptyField(&obj4) && omitEmptyLen(&obj4) == 0 {
		...
		if n != n2+4 {
			t.Fatalf("decodeIPAddr bytes read length should be %d, is %d", n2+4, n)
		}
	} else {
		if n != n2 {
			t.Fatalf("decodeIPAddr bytes read length should be %d, is %d", n2, n)
		}
	}
	...
	// DecodeExact
	var obj5 IPAddr
	if err := decodeIPAddrExact(data2, &obj5); err != nil {
		t.Fatalf("decodeIPAddr failed: %v", err)
	}
	...
	if !hasOmitEmptyField(&obj3) || omitEmptyLen(&obj3) > 0 {
		...
		if n, err := decodeIPAddr(data4, &obj3); err != nil {
			t.Fatalf("decodeIPAddr failed: %v", err)
		} else if n != uint64(len(data2)) {
			t.Fatalf("decodeIPAddr bytes read length should be %d, is %d", len(data2), n)
		}
...
func TestLaqencoderIPAddr(t *testing.T) {
	...
	cases := []testCase{
		{
			name: "empty object",
			obj:  newEmptyIPAddrForEncodeTest(),
		},
	}
	...
	for i := 0; i < nRandom; i++ {
		cases = append(cases, testCase{
			...
			obj:  newRandomIPAddrForEncodeTest(t, rand),
		})
		cases = append(cases, testCase{
			name: fmt.Sprintf("randomly populated object %d with zero length variable length contents", i),
			obj:  newRandomZeroLenIPAddrForEncodeTest(t, rand),
		})
		cases = append(cases, testCase{
			name: fmt.Sprintf("randomly populated object %d with zero length variable length contents set to nil", i),
			obj:  newRandomZeroLenNilIPAddrForEncodeTest(t, rand),
		})
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testLaqencoderIPAddr(t, tc.obj)
		})
	}
}

func decodeIPAddrExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj IPAddr
	if _, err := decodeIPAddr(buf, &obj); err == nil {
		t.Fatal("decodeIPAddr: expected error, got nil")
	} else if err != expectedErr {
		t.Fatalf("decodeIPAddr: expected error %q, got %q", expectedErr, err)
	}
}

func decodeIPAddrExactExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj IPAddr
	if err := decodeIPAddrExact(buf, &obj); err == nil {
		t.Fatal("decodeIPAddrExact: expected error, got nil")
	} else if err != expectedErr {
		t.Fatalf("decodeIPAddrExact: expected error %q, got %q", expectedErr, err)
	}
}

func testLaqencoderIPAddrDecodeErrors(t *testing.T, k int, tag string, obj *IPAddr) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
	...
	if hasOmitEmptyField(obj) && numEncodableFields(obj) > 1 {
		t.Run(fmt.Sprintf("%d %s buffer underflow nil", k, tag), func(t *testing.T) {
			decodeIPAddrExpectError(t, nil, encoder.ErrBufferUnderflow)
		})

		t.Run(fmt.Sprintf("%d %s exact buffer underflow nil", k, tag), func(t *testing.T) {
			decodeIPAddrExactExpectError(t, nil, encoder.ErrBufferUnderflow)
		})
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		t.Run(fmt.Sprintf("%d %s buffer underflow bytes=%d", k, tag, i), func(t *testing.T) {
			decodeIPAddrExpectError(t, buf[:i], encoder.ErrBufferUnderflow)
		})

		t.Run(fmt.Sprintf("%d %s exact buffer underflow bytes=%d", k, tag, i), func(t *testing.T) {
			decodeIPAddrExactExpectError(t, buf[:i], encoder.ErrBufferUnderflow)
		})
	}
	...
	t.Run(fmt.Sprintf("%d %s exact buffer remaining bytes", k, tag), func(t *testing.T) {
		decodeIPAddrExactExpectError(t, buf, encoder.ErrRemainingBytes)
	})
}

func TestLaqencoderIPAddrDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10

	for i := 0; i < n; i++ {
		emptyObj := newEmptyIPAddrForEncodeTest()
		fullObj := newRandomIPAddrForEncodeTest(t, rand)
		testLaqencoderIPAddrDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderIPAddrDecodeErrors(t, i, "full", fullObj)
	}
}
