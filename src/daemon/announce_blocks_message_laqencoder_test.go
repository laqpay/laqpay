...
func testLaqencoderAnnounceBlocksMessage(t *testing.T, obj *AnnounceBlocksMessage) {
	...
	// Encode
	data2, err := encodeAnnounceBlocksMessage(obj)
	if err != nil {
		t.Fatalf("encodeAnnounceBlocksMessage failed: %v", err)
	}
	if uint64(len(data2)) != n2 {
		t.Fatal("encodeAnnounceBlocksMessage produced bytes of unexpected length")
	}
	...
	if err := encodeAnnounceBlocksMessageToBuffer(data3, obj); err != nil {
		t.Fatalf("encodeAnnounceBlocksMessageToBuffer failed: %v", err)
	}
	...
	var obj4 AnnounceBlocksMessage
	n, err := decodeAnnounceBlocksMessage(data3, &obj4)
	if err != nil {
		t.Fatalf("decodeAnnounceBlocksMessage failed: %v", err)
	}
	...
	if hasOmitEmptyField(&obj4) && omitEmptyLen(&obj4) == 0 {
		...
		if n != n2+4 {
			t.Fatalf("decodeAnnounceBlocksMessage bytes read length should be %d, is %d", n2+4, n)
		}
	} else {
		if n != n2 {
			t.Fatalf("decodeAnnounceBlocksMessage bytes read length should be %d, is %d", n2, n)
		}
	}
	...
	// DecodeExact
	var obj5 AnnounceBlocksMessage
	if err := decodeAnnounceBlocksMessageExact(data2, &obj5); err != nil {
		t.Fatalf("decodeAnnounceBlocksMessage failed: %v", err)
	}
	...
	if !hasOmitEmptyField(&obj3) || omitEmptyLen(&obj3) > 0 {
		...
		if n, err := decodeAnnounceBlocksMessage(data4, &obj3); err != nil {
			t.Fatalf("decodeAnnounceBlocksMessage failed: %v", err)
		} else if n != uint64(len(data2)) {
			t.Fatalf("decodeAnnounceBlocksMessage bytes read length should be %d, is %d", len(data2), n)
		}
...
func TestLaqencoderAnnounceBlocksMessage(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testLaqencoderAnnounceBlocksMessage(t, tc.obj)
		})
	}
}
...
func decodeAnnounceBlocksMessageExactExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj AnnounceBlocksMessage
	if err := decodeAnnounceBlocksMessageExact(buf, &obj); err == nil {
		t.Fatal("decodeAnnounceBlocksMessageExact: expected error, got nil")
	} else if err != expectedErr {
		t.Fatalf("decodeAnnounceBlocksMessageExact: expected error %q, got %q", expectedErr, err)
	}
}

func testLaqencoderAnnounceBlocksMessageDecodeErrors(t *testing.T, k int, tag string, obj *AnnounceBlocksMessage) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
	...
	buf, err := encodeAnnounceBlocksMessage(obj)
	if err != nil {
		t.Fatalf("encodeAnnounceBlocksMessage failed: %v", err)
	}
	...
	if hasOmitEmptyField(obj) && numEncodableFields(obj) > 1 {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow nil", k, tag), func(t *testing.T) {
			decodeAnnounceBlocksMessageExactExpectError(t, nil, encoder.ErrBufferUnderflow)
		})
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow bytes=%d", k, tag, i), func(t *testing.T) {
			decodeAnnounceBlocksMessageExactExpectError(t, buf[:i], encoder.ErrBufferUnderflow)
		})
	}
	...
	t.Run(fmt.Sprintf("%d %s exact buffer remaining bytes", k, tag), func(t *testing.T) {
		decodeAnnounceBlocksMessageExactExpectError(t, buf, encoder.ErrRemainingBytes)
	})
}

func TestLaqencoderAnnounceBlocksMessageDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoderAnnounceBlocksMessageDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoderAnnounceBlocksMessageDecodeErrors(t, i, "full", fullObj)
	}
}
