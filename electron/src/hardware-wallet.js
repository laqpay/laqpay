...
// "hardware-wallet-js": "git+https://git@github.com/laqpay/hardware-wallet-js.git#ddf7265"
//
...
function checkHw(wait) {
  ...
  checkHwSubscription = Observable.of(1)
    ...
    .subscribe(
      () => {
        const connected = HID.devices().find((d) => d.manufacturer === "LaqpayFoundation");
...
ipcMain.on('hwGetDeviceConnectedSync', (event) => {
  event.returnValue = HID.devices().find((d) => d.manufacturer && d.manufacturer === "LaqpayFoundation") !== undefined;
  checkHw(false);
});
...
ipcMain.on('hwSignMessage', (event, requestId, addressIndex, message) => {
  const promise = deviceWallet.devLaqpaySignMessage(addressIndex, message, pinEvent);
  promise.then(
...
ipcMain.on('hwSignTransaction', (event, requestId, inputs, outputs) => {
  const promise = deviceWallet.devLaqpayTransactionSign(inputs, outputs, pinEvent);
  promise.then(
