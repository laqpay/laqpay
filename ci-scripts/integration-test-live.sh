...
# Runs "live"-mode tests against a laqpay node that is already running
...
# The laqpay node must be run with the wallet API enabled.
...
LAQPAY_INTEGRATION_TESTS=1 LAQPAY_INTEGRATION_TEST_MODE=$MODE LAQPAY_NODE_HOST=$HOST \
...
LAQPAY_INTEGRATION_TESTS=1 LAQPAY_INTEGRATION_TEST_MODE=$MODE RPC_ADDR=$RPC_ADDR \
	LAQPAY_NODE_HOST=$HOST USE_CSRF=$USE_CSRF HEADER_CHECK=$HEADER_CHECK LIVE_DISABLE_NETWORKING=$DISABLE_NETWORKING \
